...
///    guarantee. This covers heredocs as well: a `<<` redirect's delimiter word
///    reads as a file target. The `heredoc_defers` tests below pin that, so a
///    lexer change that stopped covering them fails here rather than silently
///    making them rewritable.
